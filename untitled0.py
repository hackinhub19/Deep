...
def get_input():
    ...
    r=pd.DataFrame({'Area 1':d[0][state],'Area 2':d[1][city],'Area 3 Classification':d[2][locality],'Outlet Type':d[3][outlet_type],'Product Category':d[4][product_category],'Product':d[5][product]})
    ...
    if predicted_revenue>threshold:
        label7['text']=('feasible with monthly revenue {}'.format(predicted_revenue))
